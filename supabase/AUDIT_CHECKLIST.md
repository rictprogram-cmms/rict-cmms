...
- [x] **Centralize super-admin exclusion.** Delivered 2026-08-24.
  ...
  - [x] Server-side exclusion done per-query instead of a view (2026-08-25): the 9 list queries that could surface the utility admin (`useWeeklyLabs`, `useEquipment`, `useLabSignup`, `useTimeCards` staff list, `useWOCRatio`, `WorkOrdersPage` assignee dropdown, `TimeCardsPage` ×2, `TimeClockPage` student picker) now carry `.neq('email', SUPER_ADMIN_EMAIL)` in the query itself. A database view was judged too risky for the benefit.
  - [x] All 24 unfiltered `profiles` queries reviewed (2026-08-25): 9 were people lists → filtered (above); the rest are single-user lookups by email/id/card, role-restricted lists (Students/Work Study/Instructor only), or the Users admin page, which intentionally shows every account.
...
- [x] **`subscribeWithReconnect()`** — 2026-08-24 (+ `usePermissions` 2× on 2026-08-25 after the supabase-js 2.112 bump surfaced a duplicate-name crash; helper now appends a sequence counter so names are always unique): 17 raw `supabase.channel()` subscriptions converted across `NotificationBell`, `usePurchaseOrders` (3), `useStudentHolds` (2), `AuthContext` (4), `InstructorToolsPage`, `WorkOrdersPage`, `ProgramCostPage`, `SettingsPage` (4), `SOPsPage`. The one remaining raw channel is the presence channel in `AuthContext` (`online-users-presence`) — it needs the channel object for `presenceState()`/`track()`, which the helper doesn't expose; leave as-is or extend the helper later.

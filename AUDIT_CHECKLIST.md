...
- [ ] **`mustData()`** on reads that gate a write or feed a user-visible number. Batch 1 (2026-08-25): 111 reads in all hooks + `AuthContext` + ID generators that previously ignored `error` (`const { data } = await …` with no error check) now go through `mustData()`. Only sites already inside a `try/catch` were converted, so a failed read surfaces through the existing handler instead of silently returning an empty list. Remaining: ~130 similar reads in pages (`WorkOrdersPage` 29, `NotificationBell` 14, `SOPsPage` 13, `TimeCardsPage` 11, `AppLayout` 9, …) — same treatment, next batch.

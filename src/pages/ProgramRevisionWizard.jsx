...
export const PROGRAMS = [
  ...
  { id: 'MECH-AAS',  name: 'Mechatronics & Automation AAS' },
  { id: 'MECH-CERT', name: 'Mechatronics & Automation Certificate' },
]
...
export default function ProgramRevisionWizard({ onClose, initialData=null, programSettings=null }) {
  ...
  // Load course catalog for the planner step (Step 5 course picker)
  // Note: program_revisions.course_id stores a program identifier (e.g. 'IPC-AAS'),
  // not a syllabus_courses.course_id — so we just load the full catalog for the
  // planner dropdown, but do NOT attempt to backfill program-level fields from it
  useEffect(()=>{
  ...
  const handleSave = async (extra={}) => {
    ...
    const { error } = await supabase.from('program_revisions').upsert(payload,{onConflict:'revision_id'}).select()
    setSaving(false)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    const { error } = await supabase.from('program_revisions').delete().eq('revision_id', data.revision_id).select()
    setDeleting(false)

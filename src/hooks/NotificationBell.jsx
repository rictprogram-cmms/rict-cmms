...
export default function NotificationBell() {
  ...
  const items = useMemo(() => [
    ...
    // time-sensitive (24-hour expiry) and there's nothing else the user
    // can do that's more important than signing for an asset waiting on them.
    ...(itemsByTable.pendingAck || []),
    ...(itemsByTable.access || []),
    ...(itemsByTable.wo || []),
    ...(itemsByTable.parts || []),
    ...(itemsByTable.time || []),
  ...
  // Pending acknowledgment modal target (student e-signature flow)
  const [ackModalTarget, setAckModalTarget] = useState(null);
  // Tick every 30s while there's at least one pending-ack item so the live
  // countdown subtitles re-render. Cheap — just a counter increment.
  const [, tickNow] = useReducer((x) => x + 1, 0);
  ...
  // Pending asset checkouts awaiting THIS user's e-signature.
  // Anyone can have one (instructors can request from each other too), so this
  // fetcher is NOT gated on isInstructor. Filter is the user's own email.
  // Subtitles are computed at render time so countdowns stay live without
  // requiring the fetcher to re-run.
  const fetchPendingAcknowledgments = useCallback(async () => {
    if (!profile?.email) {
      setItemsByTable(prev => ({ ...prev, pendingAck: [] }));
      return;
    }
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('checkout_id, asset_id, asset_name, asset_serial_number, user_id, user_email, user_name, expected_return, checkout_condition, checkout_notes, requested_by, requested_at, expires_at, status')
        .eq('user_email', myEmail)
        .eq('status', 'pending_acknowledgment')
        .order('expires_at', { ascending: true });
      if (error) console.warn('NotifBell: asset_checkouts error:', error.message);
      (data || []).forEach(r => {
        // Drop rows the client believes are already expired — server's pg_cron
        // will catch up shortly and the realtime listener will refresh.
        if (r.expires_at && new Date(r.expires_at).getTime() <= Date.now()) return;
        _items.push({
          id: `pending-ack-${r.checkout_id}`,
          type: 'pending_ack',
          icon: 'verified_user',
          color: '#228be6',
          title: r.asset_name || r.asset_id,
          // subtitle is recomputed in the render block from raw.expires_at
          subtitle: '',
          date: r.requested_at,
          raw: r,
        });
      });
    } catch (e) { console.warn('NotifBell: asset_checkouts exception:', e.message); }
    setItemsByTable(prev => ({ ...prev, pendingAck: _items }));
  }, [profile?.email]);
  ...
  useEffect(() => {
    ...
    const tableToFetcher = {
      ...
      asset_checkouts:      fetchPendingAcknowledgments,
  ...
  // bell when nothing is time-sensitive. Subtitles for pending_ack items
  // are computed in the render block from item.raw.expires_at.
  useEffect(() => {
    const pendingCount = (itemsByTable.pendingAck || []).length;
  ...
  return (
    <div ref={panelRef} style={{ position: 'relative' }}>
        ...
        <div id="notification-panel" className="nbell-panel" role="dialog" aria-modal="false" aria-labelledby="notification-panel-title">
          ...
          <div className="nbell-panel-body">
            {items.length === 0 ? (
              ...
              // Live countdown for pending acknowledgments — recomputed on every
              // render. The 30s tickNow effect drives the parent re-render so
              // the displayed minutes stay accurate without restarting fetches.
              let displaySubtitle = item.subtitle;
              if (item.type === 'pending_ack' && item.raw?.expires_at) {
                const cd = formatCountdown(item.raw.expires_at, Date.now());
                if (cd) {
                  const idPart = item.raw.asset_id ? `${item.raw.asset_id} · ` : '';
                  displaySubtitle = cd.expired
                    ? `${idPart}Expired`
                    : `${idPart}${cd.label} left to sign`;
                }
              }
              return (
              <div key={item.id} className="nbell-item">
                <div className="nbell-item-top">
                  ...
                  <div className="nbell-item-content">
                    ...
                    <div className="nbell-item-sub">{displaySubtitle}</div>
                  </div>
      ...
      {/* Pending Asset-Checkout Acknowledgment Modal — student e-signature flow.
          ...
          realtime listener removes the row from the bell list automatically. */}
      {ackModalTarget && (
        <PendingAcknowledgmentModal
          isOpen={!!ackModalTarget}
          onClose={() => setAckModalTarget(null)}
          checkout={ackModalTarget}
          userName={ackModalTarget.user_name || (profile ? `${profile.first_name || ''} ${profile.last_name || ''}`.trim() : '')}
          onAcknowledged={() => { setAckModalTarget(null); fetchPendingAcknowledgments(); }}
          onDeclined={() => { setAckModalTarget(null); fetchPendingAcknowledgments(); }}
        />
      )}
    </div>
  );
